...
const MULT_COLOR = Color.DODGER_BLUE
const SCORE_COLOR = Color.HOT_PINK
const MULT_MULT_COLOR = Color.YELLOW
...
func score_turn(
	...
	for w in words:
		...
		var label = DisappearingLabel.new("+%d MULT" % w["text"].length(), first_tile.global_position, _get_label_direction_from_word(w))
			...
			if slot_effect:
				match slot_effect.type:
					SlotEffect.SlotEffectType.MULT_MULT:
						...
						label = DisappearingLabel.new("MULT x%d" % slot_effect.value, slot.global_position,  _get_label_direction_from_word(w))
						self.add_child(label)
						turn_mult_updated.emit(turn_mul)
						var tile : GameTile = board_tiles[pos.y][pos.x]
						tile.pop(MULT_MULT_COLOR)
						AudioStreamManager.play_good_sound(pitch)
						pitch += 0.04
						await get_tree().create_timer(TIME_BETWEEN_ANIMATIONS).timeout
...
func _score_word(
	...
	for pos : Vector2i in word_info["tiles"]:
		...
		var label = _spawn_score_label(str(tile_pts), tile.global_position, _get_label_direction_from_word(word_info))
		points_updated.emit(turn_in_pts + word_pts)
		AudioStreamManager.play_good_sound(pitch)
		pitch += STEP

		tile.pop(SCORE_COLOR)			# tween runs, yields until finished
		await get_tree().create_timer(TIME_BETWEEN_ANIMATIONS).timeout
		...
		for node : RuneNode in G.current_run.rune_manager.get_runes():
			for eff : RuneEffect in node.rune.after_tile_scored(ctx):
				...
				if score_changed:
					var new_label = DisappearingLabel.new(eff.exclamation, node.global_position, Vector2.DOWN)
				...
				if score_changed or mult_changed:
					...
					AudioStreamManager.play_good_sound(pitch)
					pitch += STEP
					tile.pop(SCORE_COLOR if score_changed else MULT_COLOR)
					await get_tree().create_timer(TIME_BETWEEN_ANIMATIONS).timeout
...
func _spawn_score_label(text : String, pos : Vector2, direction : Vector2i = Vector2i.UP, parent : Node = self) -> DisappearingLabel:
	var lbl = DisappearingLabel.new(text, pos, direction)
	parent.add_child(lbl)
	return lbl

func _get_label_direction_from_word(word_info : Dictionary) -> Vector2i:
	var word_direction = word_info["dir"]
	print(word_direction)
	return Vector2i.UP if word_direction == Vector2i.RIGHT else Vector2i.RIGHT
	...
	for p:Vector2i in new_tiles:
		for dir in [Vector2i.RIGHT, Vector2i.DOWN]:
			...
			if t.length() >= 2 and not seen.has(t):
				...
				words.append({ "text":t, "tiles":coords, "dir":dir })
				seen[t] = true
	return words
