...
func animate_in():
	...
	for s in slots:
		...
		if tile:
			...
			tw.parallel().tween_property(tile, "scale", Vector2.ONE, DURATION).set_delay(delay)
...
func animate_out():
	...
	for s : SlotNode in slots:
		...
		var tile : GameTile = board_state[s.coordinates.y][s.coordinates.x].tile
		if tile: tw.parallel().tween_property(tile, "scale", Vector2.ZERO, DURATION).set_delay(delay)
		delay += DELAY_STEP
