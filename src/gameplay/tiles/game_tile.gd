...
func pop(flash_color : Color = Color.WHITE) -> void:
	_run_scale_tween(0.1, Vector2.ONE, Vector2.ONE * 1.2, false)
	_run_color_tween(0.2, flash_color, Color.GRAY, true)

func bzzt() -> void:
	_kill_tweens()
	_run_color_tween(0.25, Color.RED, Color.WHITE, false)
	_run_scale_tween(0.1, Vector2.ONE, Vector2.ONE * 1.2, false)
	_run_shake_tween(0.18, 12.0, false)

# ─────────────────────── generic tween helpers ────────────────────────
func _run_color_tween(length : float, start_color : Color, end_color : Color, wait : bool) -> void:
	if _color_tw: _color_tw.kill()
	self.self_modulate = start_color
	_color_tw = create_tween()
	_color_tw.tween_property(self, "self_modulate", end_color, length).set_ease(Tween.EASE_IN)
	$Letter.self_modulate = Color.WHITE
	_color_tw.parallel().tween_property($Letter, "self_modulate", Color.BLACK, length).set_ease(Tween.EASE_IN)
